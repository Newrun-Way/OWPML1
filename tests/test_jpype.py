...
# 3. JAVA_HOME 확인 및 자동 설정
from jpype_setup import setup_java_home, get_java_info

java_info = get_java_info()
print(f"\nJAVA_HOME (설정 전): {java_info['java_home']}")
print(f"Java 경로: {java_info['java_path']}")

# JAVA_HOME 자동 설정 시도
try:
    java_home = setup_java_home()
    print(f"[성공] JAVA_HOME 설정: {java_home}")
except Exception as e:
    print(f"[오류] JAVA_HOME 설정 실패: {e}")
    sys.exit(1)
